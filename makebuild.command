...
"${UE_INSTALL_PATH}/Engine/Build/BatchFiles/RunUAT.sh" BuildCookRun -rocket -nocompile -compileeditor -installed -nop4 -project="$WORKSPACE/$PROJECTNAME.uproject" -cook -stage -archive -archivedirectory="$WORKSPACE/ClientBuild" -package -clientconfig=Development -pak -prereqs  ${ModeString}  -allmaps -nodebuginfo -targetplatform=${TARGET} -build -target=${PROJECTNAME}Client -utf8output
